...
def getlreftreflectionpt(inp):
    equalwithin1bit = lambda a, b: bin(a ^ b).count("1") <= 1
    idx = [i for i in range(len(inp)) if equalwithin1bit(inp[i], inp[0])]
    if len(idx) > 1:
        if len(idx) % 2:
            idx = idx[:-1]
    splits = []
    for dl in idx[1:]:
        ...
        notequal_counter = 0
        while i < l:
            if not equalwithin1bit(inp[i], inp[l]):
                break
            if inp[i] != inp[l]:
                notequal_counter += 1
            l -= 1
            i += 1
        if l < i and notequal_counter == 1:
